...
uint8_t process_request(TCP_CONFIG *config)
{
	...
	char *search;
	char *search2;
	search = strstr((char *)tcp_data_buffer,"yesturnon");
	search2 = strstr((char *)tcp_data_buffer,"yesturnoff");

	if(search == 0){
		bytes_to_send = strlen((char *)website);
		rets = send(config->s, website,bytes_to_send , 0);
		//LPC_GPIO1 -> FIOPIN ^= (1<<20);


	}else if(search != 0){
		bytes_to_send = strlen((char *)websiteone);
		rets = send(config->s, websiteone,bytes_to_send , 0);
		LPC_GPIO1 -> FIOPIN ^= (1<<20);
	}
	else if(search2 == 0){
		bytes_to_send = strlen((char *)websiteone);
